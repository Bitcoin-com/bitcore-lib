...
var sighashBCC = function sighashBCC(transaction, sighashType, inputNumber, scriptCode, satoshisBuffer) {
  ...
  var txLength = scriptCode.toBuffer().length;
  if (txLength < Opcode.OP_PUSHDATA1) {
    writer.writeUInt8(txLength);
  } else if (txLength <= 0xff) {
    writer.writeUInt8(Opcode.OP_PUSHDATA1);
    writer.writeUInt8(txLength);
  } else if (txLength <= 0xffff) {
    writer.writeUInt8(Opcode.OP_PUSHDATA2);
    writer.writeUInt16LE(txLength);
  } else {
    writer.writeUInt8(Opcode.OP_PUSHDATA4);
    writer.writeUInt32LE(txLength);
  }
